...
8. **Data sourcing discipline applies to the whole pipeline**, not just Trend Scout: Etsy Open API first (if `ETSY_API_KEY` is set), search-with-resolvable-URL second, explicit `NO DATA AVAILABLE` third. Never let a fabricated placeholder reach the tracker or the report, from you or from any sub-agent's report to you. **If Trend Scout reports an API error (rate limit, auth failure, non-200), surface the exact error in the CEO Summary — never let a silent fallback to search-scraping pass without telling the user it happened and why.**
9. **Every cycle's `### CEO Summary` heading must be followed immediately by a `**Run log:**` line** in this exact format (the dashboard generator parses it): `**Run log:** completed YYYY-MM-DD · next cycle scheduled YYYY-MM-DD · Agent Name: success|error (optional detail) · ...` — one `Agent Name: status` entry per sub-agent you delegated to this cycle. If a sub-agent errored or you skipped it, say so here (`error` / `skipped`), don't omit it.
10. **After writing `niche-tracker.md`/`report.html` and pushing, always run `python3 generate_dashboard.py`** (from the repo root). This regenerates `index.html` — the single-file dashboard (Live Niches / Design Briefs / History / Pipeline Status tabs) — entirely from `niche-tracker.md`, and opens it in the user's default browser. Never hand-edit `index.html` or `dashboard_template.html`'s injected data; if the dashboard looks wrong, the fix belongs in `niche-tracker.md`'s structure or in `generate_dashboard.py`'s parsing, not in the generated file. If the script exits non-zero (it fails loudly on a parse error rather than emitting a guessed dashboard), report the exact error to the user instead of leaving a stale `index.html` in place unexplained.
